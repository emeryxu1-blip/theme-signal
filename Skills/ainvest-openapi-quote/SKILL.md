...
For actual local execution, use `scripts/fetch_quote.py`. It reads one request body from `--template`, `--body-file`, or `--body-json`, infers the endpoint when possible, and sends the request for `--scene sandbox|b|c`. By default it also loads the shared local `Skills/env.json` config for `active_scene`, B-side apikey, C-side cookie fields, scene endpoints, and default headers; command-line auth values override the file. Use `--dry-run` to print the endpoint, redacted headers, and body without sending a request.
