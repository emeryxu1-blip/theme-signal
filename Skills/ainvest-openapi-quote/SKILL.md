...
For actual local execution, use `scripts/fetch_quote.py`. It reads one request body from `--template`, `--body-file`, or `--body-json`, infers the endpoint when possible, and sends the request for `--scene sandbox|b|c`. By default it also loads the shared local `Skills/env.json` config for `active_scene`, B-side apikey, manually saved C-side `cookie` or `sessionid`/`userid` fields, scene endpoints, and default headers; command-line auth values override the file. C-side authentication is never refreshed automatically and never invokes `theme_investing` login code: pass a complete Cookie with `--auth-value`, save it as `cookie`, or save manually obtained `sessionid` and `userid` values. Use `--dry-run` to print the endpoint, redacted headers, and body without sending a request.
...
- Direct C-side fetches require a caller-provided `Cookie` header, either passed with `--auth-value` or composed from manually saved `cookie` or `sessionid`/`userid` fields in `Skills/env.json`; do not attempt automatic login or session refresh and do not read it from a default environment variable.
...
- `scripts/fetch_quote.py`: execute one quote request from a template or JSON body. Supports `--scene sandbox|b|c`; sandbox uses `AIME_API_KEY`, B-side accepts `--index-api-apikey` or `--quoteag-apikey`, and C-side accepts a complete Cookie via `--auth-value`. B-side auth and manually saved C-side `cookie` or `sessionid`/`userid` can also come from the shared local `Skills/env.json`; the script never performs C-side login or session refresh.
